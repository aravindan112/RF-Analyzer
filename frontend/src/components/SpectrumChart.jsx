...
export default function SpectrumChart({
  ...
  const fetchFreqSpan = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(ids.map(id => {
        const qs = new URLSearchParams({ dataset_id:id, position_ms:p.positionMs,
  ...
  const fsTraces = Object.entries(datasets).flatMap(([id,d],idx) => {
    ...
    if (isMain && d.peak_mhz!=null) out.push({
  ...
  const primary=datasets[activeDatasetId], fsShapes=[], fsAnnotations=[];
  ...
  if (zsData?.times_ms?.length && zsData?.powers_db?.length) {
    zsTraces.push({ x:zsData.times_ms, y:zsData.powers_db, type:'scatter', mode:'lines',
    ...
    if (pkIdx>=0) zsTraces.push({ x:[zsData.times_ms[pkIdx]],y:[pkPwr],type:'scatter',mode:'markers',
  ...
  // ── Layouts ───────────────────────────────────────────────────────────────
  const base = { paper_bgcolor:'#0a0e1a', plot_bgcolor:'#080c18', margin:{l:56,r:20,t:7,b:32},
    width:dims.width, height:dims.height, dragmode:'zoom',
    modebar:{bgcolor:'transparent',color:'#3a5070',activecolor:'#00d4ff'} };

  const fsLayout = { ...base, uirevision:'spectrum',
  ...
  const zsLayout = { ...base, uirevision:'zero_span',
        ...
        {mode==='zero' && <>
          ...
          {!isRunning ? (
            <button onClick={zs_start} disabled={!activeDatasetId}
              style={{ padding:'3px 18px', fontSize:10, fontWeight:700, ...mono, cursor:activeDatasetId?'pointer':'not-allowed',
            ...
            <button onClick={zs_stop}
              style={{ padding:'3px 18px', fontSize:10, fontWeight:700, ...mono, cursor:'pointer',
